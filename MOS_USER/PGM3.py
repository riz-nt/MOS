...
def simulate_paging(processes, memory_size):
    num_frames = memory_size // len(processes)  # Equal allocation of frames for each process
...
if __name__ == "__main__":
    ...
    for i in range(num_processes):
        process_id = i + 1
        pages = input(f"Enter pages for process {process_id} separated by space: ").split()
        processes.append(Process(process_id, pages))
